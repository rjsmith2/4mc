...
NBT = function(nbtfile) {
	...
	nbtproto.loadFile = function(nbtfile, compressed) {
		...
		var filestring = new StringStream(nbtfile.data.join("")); //why [0]?
	...
	function getarrG(){return arrG.join("\n");}
	nbtproto.readType = function(nbtfile, type) {
		arrG.push("type is "+type);
		switch(type) {
			...
			case this.tags.TAG_INT_ARRAY://new taag
				arrG.push("trying");
				
				var arrayLength = this.readType(nbtfile, this.tags.TAG_INT);
				var byteArray = new Array();
				for(var i = 0; i < arrayLength; i++) byteArray.push(this.readType(nbtfile, this.tags.TAG_INT));
				return byteArray;
